...
{
    var allowedDirectories = new[] { Environment.CurrentDirectory, AppContext.BaseDirectory };
    ...
    if (!string.IsNullOrWhiteSpace(explicitPath))
    {
        ...
        {
            var allowedDir = Path.GetFullPath(dir);
            if (!allowedDir.EndsWith(Path.DirectorySeparatorChar))
            {
                allowedDir += Path.DirectorySeparatorChar;
            }

            if (fullPath.StartsWith(allowedDir, StringComparison.OrdinalIgnoreCase) ||
                string.Equals(fullPath, allowedDir.TrimEnd(Path.DirectorySeparatorChar), StringComparison.OrdinalIgnoreCase))
            {
                isAllowed = true;
                break;
            }
